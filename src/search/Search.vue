<template>
  ...
  <div>
    <ul>
      <li v-for="(result, index) in pagedResults" :key="index"><!--zamiast array.map-->
...
export default { // przyklad Composition API i Reactivity
// -->podobne do customHookow , seprowanie logik do : search , pagination, filter ,
  setup(props) { // zastepuje stare metody data, computed , mounted, beforeDestroyed etc
  // mozna laczyc ze starymi tylko po co ?
    const { searchResults, search } = useSearch(props.searchTerm);

    const {
      filters,
      applyFilters,
      clearFilters, // destrukturuzyjemy sobie z "use" to co chcemy
      filteredResults,
    ...
    const resultCount = computed(() => filteredResults.value.length);// wartos która korzysta
    // z innych zmiennych,
    // zby sie rowniez przeliczała
